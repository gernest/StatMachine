...
func TestHighestScoring(t *testing.T){
	...
	if(!(5==goalsScored(highestScoring[0]) && 2==goalsConceded(highestScoring[0]))){
...
func TestBiggestScoringResultsIfMultiple(t *testing.T){
	...
	for _,r := range highestScoring{
		if(goalsScored(r)+goalsConceded(r)!=4){
			t.Errorf("Got a result with unexpected goal sum in TestBiggestScoringResults, got %v, expected 4",goalsScored(r)+goalsConceded(r)) 
		}
	}
}